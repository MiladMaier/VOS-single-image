...
# Make local detectron2 importable when running scripts from the repo root.
# This prepends the `detectron2` directory (if present) to sys.path so
# `import detectron2` resolves to the local copy instead of failing.
_local_d2 = os.path.join(core.top_dir(), 'detectron2')
if os.path.isdir(_local_d2) and _local_d2 not in sys.path:
    sys.path.insert(0, _local_d2)

# Ensure repo root is on sys.path so imports that expect project-root
# modules (like `calibration.py` at the repo root) are resolvable when
# running this script from the `detection/` directory.
_repo_root = core.top_dir()
