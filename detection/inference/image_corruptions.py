...
try:
    # Optional dependency: Wand (Python bindings for ImageMagick).
    # Wand requires the ImageMagick shared library to be installed on the
    # system. If it's not available we provide a lightweight fallback
    # MotionImage class so the module can still be imported and functions
    # that rely on wand will become no-ops (useful for running without
    # the ImageMagick dependency when image corruptions are disabled).
...
if _HAS_WAND:
    ...
    # Minimal fallback MotionImage: wraps a blob and provides make_blob()
    # so later code can still call MotionImage(blob=...).motion_blur()
    # and then make_blob(); motion_blur is a no-op here.
    class MotionImage:
        ...
        def make_blob(self):
            # return original blob if present, otherwise empty bytes
            return self._blob if self._blob is not None else b''
